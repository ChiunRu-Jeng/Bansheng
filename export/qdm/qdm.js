...
(function () {
  ...
  var ballPh = document.getElementById('ballPh');
  var ballKo = document.getElementById('ballKo');
  var ballName = document.getElementById('ballName');
  var ballDesc = document.getElementById('ballDesc');
  document.querySelectorAll('.flavor').forEach(function (f) {
    f.addEventListener('click', function () {
      var act = document.querySelector('.flavor.active');
      if (act) act.classList.remove('active');
      f.classList.add('active');
      var d = flavorData[+f.dataset.i];
      if (ballPh) {
        ballPh.style.backgroundColor = d.color;
        var s = ballPh.querySelector('span');
        if (s) s.textContent = '[ 乳酪球 · ' + d.name + ' ]';
      }
      if (ballKo) ballKo.textContent = d.ko;
      if (ballName) { ballName.textContent = d.name; ballName.style.color = d.color; }
      if (ballDesc) ballDesc.textContent = d.desc;
    });
  });
